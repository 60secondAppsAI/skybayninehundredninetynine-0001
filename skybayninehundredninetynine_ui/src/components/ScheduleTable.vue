        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalSchedules = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalSchedules">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Schedule</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ScheduleId" type="text" placeholder="Enter ScheduleId" v-model="scheduleToAdd.scheduleId"></base-input>
  <base-input label="ScheduledDate" type="text" placeholder="Enter ScheduledDate" v-model="scheduleToAdd.scheduledDate"></base-input>
  <base-input label="Status" type="text" placeholder="Enter Status" v-model="scheduleToAdd.status"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="schedules" :row-key="record => record.ScheduleId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <SchedulePictureView :schedules="schedules" />
          </div>
...
import SchedulePictureView from './SchedulePictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'scheduleId',
          visible: true,
          scopedSlots: { customRender: 'scheduleId' },
          sorter: true
          //sorter: (a, b) => a.scheduleId - b.scheduleId,
          //sorter: (a, b) => a.scheduleId.localeCompare(b.scheduleId),
        },
        {
		...
		dataIndex: 'scheduledDate',
          ...
          sorter: true
          //sorter: (a, b) => a.scheduledDate - b.scheduledDate,
      ...
      sortBy: 'scheduleId',           // Column to sort by
  ...
  methods: {
    ...
    async renderSchedulesTable() {
      ...
      let schedulesTableData = [];
      for (let i = 0; i < this.schedules.length; i++) {
        schedulesTableData.push({
          id: i,
          scheduleId: this.schedules[i].scheduleId,
          year: this.schedules[i].year,
          date: this.schedules[i].date,
          competitionId: this.schedules[i].competitionId,
          scheduleId: this.schedules[i].scheduleId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-schedules',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForSchedulesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalSchedules = false;

      const currentDate = new Date().getTime();
      this.scheduleToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.scheduleToAdd);
      console.log(jsonData);
      
      const res = await ScheduleService.addSchedule(jsonData);
