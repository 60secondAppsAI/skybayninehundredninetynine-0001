        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalMeals = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalMeals">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Meal</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="MealId" type="text" placeholder="Enter MealId" v-model="mealToAdd.mealId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="mealToAdd.name"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="mealToAdd.description"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="meals" :row-key="record => record.MealId" :pagination="pagination"
...
import MealPictureView from './MealPictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'mealId',
          visible: true,
          scopedSlots: { customRender: 'mealId' },
          sorter: true
          //sorter: (a, b) => a.mealId - b.mealId,
          //sorter: (a, b) => a.mealId.localeCompare(b.mealId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      sortBy: 'mealId',           // Column to sort by
  ...
  methods: {
    ...
    async renderMealsTable() {
      ...
      for (let i = 0; i < this.meals.length; i++) {
        mealsTableData.push({
          id: i,
          mealId: this.meals[i].mealId,
          year: this.meals[i].year,
          date: this.meals[i].date,
          competitionId: this.meals[i].competitionId,
          mealId: this.meals[i].mealId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-meals',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForMealsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalMeals = false;

      const currentDate = new Date().getTime();
      this.mealToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.mealToAdd);
      console.log(jsonData);
      
      const res = await MealService.addMeal(jsonData);
