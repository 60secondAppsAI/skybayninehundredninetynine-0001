        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Airplanes</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalAirplanes = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAirplanes">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Airplane</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AirplaneId" type="text" placeholder="Enter AirplaneId" v-model="airplaneToAdd.airplaneId"></base-input>
  <base-input label="Model" type="text" placeholder="Enter Model" v-model="airplaneToAdd.model"></base-input>
  <base-input label="Manufacturer" type="text" placeholder="Enter Manufacturer" v-model="airplaneToAdd.manufacturer"></base-input>
  <base-input label="SeatCapacity" type="text" placeholder="Enter SeatCapacity" v-model="airplaneToAdd.seatCapacity"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="airplanes" :row-key="record => record.AirplaneId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AirplanePictureView :airplanes="airplanes" />
          </div>
...
import AirplanePictureView from './AirplanePictureView.vue';


const airplanesColumns = [
  "airplaneId",
  "year",
  "date",
  "competitionId",
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'airplaneId',
          visible: true,
          scopedSlots: { customRender: 'airplaneId' },
          sorter: true
          //sorter: (a, b) => a.airplaneId - b.airplaneId,
          //sorter: (a, b) => a.airplaneId.localeCompare(b.airplaneId),
        },
        {
          title: 'Model',
		dataIndex: 'model',
		...
		dataIndex: 'seatCapacity',
          visible: true,
          scopedSlots: { customRender: 'seatCapacity' },
          sorter: true
          //sorter: (a, b) => a.seatCapacity - b.seatCapacity,
          //sorter: (a, b) => a.seatCapacity.localeCompare(b.seatCapacity),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} airplanes`,
      },
      ...
      airplanesTable: {
        columns: [...airplanesColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'airplaneId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAirplanesTable() {
      ...
      let airplanesTableData = [];
      for (let i = 0; i < this.airplanes.length; i++) {
        airplanesTableData.push({
          id: i,
          airplaneId: this.airplanes[i].airplaneId,
          year: this.airplanes[i].year,
          date: this.airplanes[i].date,
          competitionId: this.airplanes[i].competitionId,
          airplaneId: this.airplanes[i].airplaneId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-airplanes',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAirplanesChanged', this.searchQuery);
		//this.renderAirplanesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalAirplanes = false;

      const currentDate = new Date().getTime();
      this.airplaneToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.airplaneToAdd);
      console.log(jsonData);
      
      const res = await AirplaneService.addAirplane(jsonData);
