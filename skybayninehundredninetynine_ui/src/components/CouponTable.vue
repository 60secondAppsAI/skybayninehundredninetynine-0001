...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalCoupons = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCoupons">
                <template slot="header">
  ...
  <base-input label="ExpirationDate" type="text" placeholder="Enter ExpirationDate" v-model="couponToAdd.expirationDate"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="coupons" :row-key="record => record.CouponId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <CouponPictureView :coupons="coupons" />
          </div>
...
import CouponPictureView from './CouponPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'couponId',
          ...
          //sorter: (a, b) => a.couponId.localeCompare(b.couponId),
        },
        {
          title: 'Code',
		dataIndex: 'code',
		...
		dataIndex: 'discountPercentage',
          ...
          //sorter: (a, b) => a.discountPercentage.localeCompare(b.discountPercentage),
        },
        {
      ...
      sortBy: 'couponId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCouponsTable() {
      ...
      let couponsTableData = [];
      for (let i = 0; i < this.coupons.length; i++) {
        couponsTableData.push({
          id: i,
          couponId: this.coupons[i].couponId,
          year: this.coupons[i].year,
          date: this.coupons[i].date,
          competitionId: this.coupons[i].competitionId,
          couponId: this.coupons[i].couponId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-coupons',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCouponsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalCoupons = false;

      const currentDate = new Date().getTime();
      this.couponToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.couponToAdd);
      console.log(jsonData);
      
      const res = await CouponService.addCoupon(jsonData);
