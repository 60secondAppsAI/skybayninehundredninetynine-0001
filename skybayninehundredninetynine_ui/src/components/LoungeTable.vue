        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Lounges</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalLounges = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalLounges">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Lounge</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="LoungeId" type="text" placeholder="Enter LoungeId" v-model="loungeToAdd.loungeId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="loungeToAdd.name"></base-input>
  <base-input label="Services" type="text" placeholder="Enter Services" v-model="loungeToAdd.services"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="lounges" :row-key="record => record.LoungeId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <LoungePictureView :lounges="lounges" />
          </div>
...
import LoungePictureView from './LoungePictureView.vue';


const loungesColumns = [
  "loungeId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'loungeId',
          visible: true,
          scopedSlots: { customRender: 'loungeId' },
          sorter: true
          //sorter: (a, b) => a.loungeId - b.loungeId,
          //sorter: (a, b) => a.loungeId.localeCompare(b.loungeId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} lounges`,
      },
      ...
      loungesTable: {
        columns: [...loungesColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'loungeId',           // Column to sort by
  ...
  methods: {
    ...
    async renderLoungesTable() {
      ...
      let loungesTableData = [];
      for (let i = 0; i < this.lounges.length; i++) {
        loungesTableData.push({
          id: i,
          loungeId: this.lounges[i].loungeId,
          year: this.lounges[i].year,
          date: this.lounges[i].date,
          competitionId: this.lounges[i].competitionId,
          loungeId: this.lounges[i].loungeId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-lounges',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForLoungesChanged', this.searchQuery);
		//this.renderLoungesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalLounges = false;

      const currentDate = new Date().getTime();
      this.loungeToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.loungeToAdd);
      console.log(jsonData);
      
      const res = await LoungeService.addLounge(jsonData);
