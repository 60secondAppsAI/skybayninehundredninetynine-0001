        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalBookings = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalBookings">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Booking</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="BookingId" type="text" placeholder="Enter BookingId" v-model="bookingToAdd.bookingId"></base-input>
  <base-input label="BookingDate" type="text" placeholder="Enter BookingDate" v-model="bookingToAdd.bookingDate"></base-input>
  <base-input label="TotalPrice" type="text" placeholder="Enter TotalPrice" v-model="bookingToAdd.totalPrice"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="bookings" :row-key="record => record.BookingId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <BookingPictureView :bookings="bookings" />
          </div>
...
import BookingPictureView from './BookingPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      sortBy: 'bookingId',           // Column to sort by
  ...
  methods: {
    ...
    async renderBookingsTable() {
      ...
      for (let i = 0; i < this.bookings.length; i++) {
        bookingsTableData.push({
          id: i,
          bookingId: this.bookings[i].bookingId,
          year: this.bookings[i].year,
          date: this.bookings[i].date,
          competitionId: this.bookings[i].competitionId,
          bookingId: this.bookings[i].bookingId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-bookings',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForBookingsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalBookings = false;

      const currentDate = new Date().getTime();
      this.bookingToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.bookingToAdd);
      console.log(jsonData);
      
      const res = await BookingService.addBooking(jsonData);
