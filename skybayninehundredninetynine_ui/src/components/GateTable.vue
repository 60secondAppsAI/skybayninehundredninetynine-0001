        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalGates = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalGates">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Gate</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="GateId" type="text" placeholder="Enter GateId" v-model="gateToAdd.gateId"></base-input>
  <base-input label="GateNumber" type="text" placeholder="Enter GateNumber" v-model="gateToAdd.gateNumber"></base-input>
  <base-input label="Terminal" type="text" placeholder="Enter Terminal" v-model="gateToAdd.terminal"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="gates" :row-key="record => record.GateId" :pagination="pagination"
...
import GatePictureView from './GatePictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'gateId',
          visible: true,
          scopedSlots: { customRender: 'gateId' },
          sorter: true
          //sorter: (a, b) => a.gateId - b.gateId,
          //sorter: (a, b) => a.gateId.localeCompare(b.gateId),
        },
        {
          title: 'Gate Number',
		dataIndex: 'gateNumber',
          visible: true,
          scopedSlots: { customRender: 'gateNumber' },
          sorter: true
          //sorter: (a, b) => a.gateNumber - b.gateNumber,
          //sorter: (a, b) => a.gateNumber.localeCompare(b.gateNumber),
        },
        {
          title: 'Terminal',
		dataIndex: 'terminal',
          ...
          sorter: true
          //sorter: (a, b) => a.terminal - b.terminal,
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} gates`,
      },
      ...
      sortBy: 'gateId',           // Column to sort by
  ...
  methods: {
    ...
    async renderGatesTable() {
      ...
      for (let i = 0; i < this.gates.length; i++) {
        gatesTableData.push({
          id: i,
          gateId: this.gates[i].gateId,
          year: this.gates[i].year,
          date: this.gates[i].date,
          competitionId: this.gates[i].competitionId,
          gateId: this.gates[i].gateId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-gates',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForGatesChanged', this.searchQuery);
		//this.renderGatesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalGates = false;

      const currentDate = new Date().getTime();
      this.gateToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.gateToAdd);
      console.log(jsonData);
      
      const res = await GateService.addGate(jsonData);
