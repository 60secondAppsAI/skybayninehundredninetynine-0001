        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalAirports = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAirports">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Airport</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AirportId" type="text" placeholder="Enter AirportId" v-model="airportToAdd.airportId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="airportToAdd.name"></base-input>
  <base-input label="City" type="text" placeholder="Enter City" v-model="airportToAdd.city"></base-input>
  <base-input label="Country" type="text" placeholder="Enter Country" v-model="airportToAdd.country"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="airports" :row-key="record => record.AirportId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AirportPictureView :airports="airports" />
          </div>
...
import AirportPictureView from './AirportPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'airportId',
          visible: true,
          scopedSlots: { customRender: 'airportId' },
          sorter: true
          //sorter: (a, b) => a.airportId - b.airportId,
          //sorter: (a, b) => a.airportId.localeCompare(b.airportId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      sortBy: 'airportId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAirportsTable() {
      ...
      let airportsTableData = [];
      for (let i = 0; i < this.airports.length; i++) {
        airportsTableData.push({
          id: i,
          airportId: this.airports[i].airportId,
          year: this.airports[i].year,
          date: this.airports[i].date,
          competitionId: this.airports[i].competitionId,
          airportId: this.airports[i].airportId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-airports',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAirportsChanged', this.searchQuery);
		//this.renderAirportsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalAirports = false;

      const currentDate = new Date().getTime();
      this.airportToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.airportToAdd);
      console.log(jsonData);
      
      const res = await AirportService.addAirport(jsonData);
